...
const WorshipPlayerContext = createContext<WorshipPlayerContextType | null>(
  null,
);
...
export function useWorshipPlayer() {
  ...
  if (!context) {
    throw new Error(
      "useWorshipPlayer must be used within a WorshipPlayerProvider",
    );
  }
  return context;
}
...
function PlayerPortal({
  ...
  const isMiniMode =
    showMiniPlayer && !mainPlayerVisible && currentVideo && !iOSModalMode;
...
export function WorshipPlayerProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadLastPlayed = async () => {
      try {
        const response = await fetch("/api/user/last-played", {
          credentials: "include",
        });
        if (response.ok) {
  ...
  useEffect(() => {
    ...
    const loadLoopingPreference = async () => {
      try {
        const response = await fetch("/api/user/looping-preference", {
          credentials: "include",
        });
        if (response.ok) {
  ...
  useEffect(() => {
    ...
    const loadVolumePreference = async () => {
      try {
        const response = await fetch("/api/user/volume-preference", {
          credentials: "include",
        });
        if (response.ok) {
  ...
  const showMiniPlayer =
    miniPlayerActivated && !mainPlayerVisible && !miniPlayerDismissed;
  ...
  useEffect(() => {
    const loadAPI = () => {
      ...
      logEvent("YT_API_SCRIPT_CHECK", {
        payload: { existingScript: !!existingScript },
      });
      ...
      window.onYouTubeIframeAPIReady = () => {
        logEvent("YT_API_READY_CALLBACK", {
          payload: { hasYT: !!window.YT, hasPlayer: !!window.YT?.Player },
        });
        setApiLoaded(true);
      };
    };
        ...
        events: {
          ...
          onReady: (event) => {
            ...
            try {
              ...
              logEvent("YT_ON_READY_GET_STATE_ERROR", {
  ...
  useEffect(() => {
    ...
    trackChangeDebounceRef.current = setTimeout(() => {
      ...
      if (shouldAutoPlay) {
        ...
        logEvent("CUE_VIDEO_BY_ID", {
          videoId: currentVideo.youtubeVideoId,
          videoIndex: currentIndex,
          payload: {
            autoPlay: false,
            loadedVideoIdStays: loadedVideoIdRef.current,
          },
        });
  ...
  const play = useCallback(() => {
    ...
    const initTimeoutId = setTimeout(() => {
      logEvent("PLAY_TIMEOUT", {
        videoId,
        videoIndex: currentIndex,
        payload: { reason: "5s timeout reached" },
      });
      setIsInitializing(false);
    }, 5000);
    ...
    logEvent("PLAY_QUEUE_PENDING", {
      videoId,
      videoIndex: currentIndex,
      payload: { playerExists: !!playerRef.current, playerReady },
    });
    pendingPlayCallbackRef.current = executePlay;
    autoPlayOnReadyRef.current = true;
  ...
  const pause = useCallback(() => {
    logEvent("PAUSE_CALLED", {
      videoId: videos[currentIndex]?.youtubeVideoId,
      videoIndex: currentIndex,
    });
  ...
  useEffect(() => {
    if (isPlaying && iOSModalVisible) {
      logEvent("IOS_MODAL_AUTO_CLOSE", {
        payload: { reason: "playback_started" },
      });
      setIOSModalVisible(false);
    }
          ...
          >
            ...
            <div className="absolute bottom-8 left-4 right-4 text-center">
              ...
              <p className="text-white/50 text-sm mt-1">
                Tap the play button, then close this
              </p>
            </div>
          </div>,
          document.body,
